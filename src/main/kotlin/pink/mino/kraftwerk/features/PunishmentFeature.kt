        ...
        fun punish(player: OfflinePlayer, punishment: Punishment) {
            if (player.isOnline) {
                ...
                if (punishment.type == PunishmentType.WARN) {
                    (player as Player).showTitle(Title.title(Chat.colored("${Chat.secondaryColor}WARNING!"), Chat.colored("<gray>${punishment.reason}")))
                    Chat.sendMessage(player, Chat.line)
                    Chat.sendMessage(player, "${Chat.secondaryColor}<bold>WARNING!")
                    Chat.sendMessage(player, " ")
                    Chat.sendMessage(player, "<gray>You've been warned by ${Chat.secondaryColor}${Bukkit.getOfflinePlayer(punishment.punisherUuid).name}<gray>!")
                    Chat.sendMessage(player, "<gray>Further warnings may result in further action.")
                    Chat.sendMessage(player, "<gray>Reason: ${Chat.secondaryColor}${punishment.reason}")
                    Chat.sendMessage(player, Chat.line)
                    player.playSound(player.location, Sound.BLOCK_ANVIL_LAND, 1f, 1f)
                }
                if (punishment.type == PunishmentType.DISQUALIFICATION) {
                    (player as Player).showTitle(Title.title(Chat.colored("${Chat.secondaryColor}DISQUALIFIED!"), Chat.colored("<gray>${punishment.reason})")))
                    Chat.sendMessage(player, Chat.line)
                    Chat.sendMessage(player, "${Chat.secondaryColor}<bold>DISQUALIFIED!")
                    Chat.sendMessage(player, " ")
                    Chat.sendMessage(player, "<gray>You've been disqualified by ${Chat.secondaryColor}${Bukkit.getOfflinePlayer(punishment.punisherUuid).name}<gray>!")
                    Chat.sendMessage(player, "<gray>Reason: ${Chat.secondaryColor}${punishment.reason}")
                    Chat.sendMessage(player, Chat.line)
                ...
                if (punishment.type == PunishmentType.HELPOP_MUTE) {
                    ...
                    Chat.sendMessage(player, "${Chat.secondaryColor}<bold>HELPOP MUTED!")
                    Chat.sendMessage(player, " ")
                    Chat.sendMessage(player, "<gray>You've been helpop muted by ${Chat.secondaryColor}${Bukkit.getOfflinePlayer(punishment.punisherUuid).name}<gray>!")
                    Chat.sendMessage(player, "<gray>You may no longer use ${Chat.secondaryColor}/helpop<gray> to ask for help from Staff.")
                    Chat.sendMessage(player, "<gray>This punishment expires in: ${Chat.secondaryColor}${timeToString(punishment.expiresAt - System.currentTimeMillis())}<gray>.")
                    Chat.sendMessage(player, "<gray>Reason: ${Chat.secondaryColor}${punishment.reason}")
                    Chat.sendMessage(player, Chat.line)
                ...
                if (punishment.type == PunishmentType.MUTE) {
                    Chat.sendMessage((player as Player), Chat.line)
                    Chat.sendMessage(player, "${Chat.secondaryColor}<bold>MUTED!")
                    Chat.sendMessage(player, " ")
                    Chat.sendMessage(player, "<gray>You've been muted by ${Chat.secondaryColor}${Bukkit.getOfflinePlayer(punishment.punisherUuid).name}<gray>!")
                    Chat.sendMessage(player, "<gray>You may no longer talk in chat.")
                    Chat.sendMessage(player, "<gray>This punishment expires in: ${Chat.secondaryColor}${timeToString(punishment.expiresAt - System.currentTimeMillis())}<gray>.")
                    Chat.sendMessage(player, "<gray>Reason: ${Chat.secondaryColor}${punishment.reason}")
                    Chat.sendMessage(player, Chat.line)
                    player.playSound(player.location, Sound.BLOCK_NOTE_BLOCK_BASS, 1f, 1f)
