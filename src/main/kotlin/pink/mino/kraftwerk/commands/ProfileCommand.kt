...
class ProfileCommand : CommandExecutor {
    override fun onCommand(
        ...
        val stats = ItemBuilder(Material.DIAMOND)
            .name(" ${Chat.primaryColor}<bold>Statistics")
            .addLore(" ")
        ...
        val misc = ItemBuilder(Material.NETHER_STAR)
            ...
            .addLore(" <gray>Coins ${Chat.dash} <gold>⚜ ${floor(profile.coins)}")
            .addLore(" ")
            .make()
        ...
        gui.item(11, settings).onClick runnable@ { inventoryClickEvent ->
            ...
            if (profile.disableRedstonePickup) {
                disableRedstonePickup.name("<green><bold>Disable Redstone Pickup")
                    ...
                    .addLore("<dark_gray><italic>Click to toggle!")
            } else {
                disableRedstonePickup.name("<red><bold>Disable Redstone Pickup")
                    .addLore("<gray>Currently ${Chat.dash} <red>Disabled")
                    .addLore("<dark_gray><italic>Click to toggle!")
            }
            val disableLapisPickup = ItemBuilder(Material.INK_SAC)
                .setDurability(4)
            if (profile.disableLapisPickup) {
                disableLapisPickup.name("<green><bold>Disable Lapis Pickup")
                    .addLore("<gray>Currently ${Chat.dash} <green>Enabled")
                    .addLore("<dark_gray><italic>Click to toggle!")
            } else {
                disableLapisPickup.name("<red><bold>Disable Lapis Pickup")
                    .addLore("<gray>Currently ${Chat.dash} <red>Disabled")
                    .addLore("<dark_gray><italic>Click to toggle!")
            }
            val projectileMessages = ItemBuilder(Material.ARROW)
                .name("<yellow><bold>Projectile Messages")
            ...
            val borderPreference = ItemBuilder(Material.BEDROCK)
                .name("<yellow><bold>Border Preference")
                .addLore("<gray>Currently ${Chat.dash} ${Chat.secondaryColor}${profile.borderPreference}")
                .addLore("<dark_gray><italic>Click to toggle!")
                .make()
            val deathMessageOnScreen = ItemBuilder(Material.WRITABLE_BOOK)
            if (profile.deathMessageOnScreen) {
                deathMessageOnScreen.name("<green><bold>Death Message on Screen")
                    .addLore("<gray>Currently ${Chat.dash} <green>Enabled")
                    .addLore("<dark_gray><italic>Click to toggle!")
            } else {
                deathMessageOnScreen.name("<red><bold>Death Message on Screen")
                    .addLore("<gray>Currently ${Chat.dash} <red>Disabled")
                    .addLore("<dark_gray><italic>Click to toggle!")
            }
            ...
            gui.item(0, disableRedstonePickup.make()).onClick runnable@ {
                if (!PerkChecker.checkPerks(player).contains(Perk.TOGGLE_PICKUPS)) {
                    Chat.sendMessage(player, "<red>This setting is locked to <dark_green>Donator<red> ranks. <red>Buy it at <yellow>${if (ConfigFeature.instance.config!!.getString("chat.storeUrl") != null) ConfigFeature.instance.config!!.getString("chat.storeUrl") else "no store url setup in config tough tits"}<red>.")
                    return@runnable
                }
                if (profile.disableRedstonePickup) {
        ...
        val display = ItemBuilder(Material.REDSTONE)
            .name(" ${Chat.primaryColor}<bold>Display")
            .addLore(" ")
        ...
        gui.item(15, display).onClick runnable@ {
            ...
            val arenaBlocks = ItemBuilder(Material.COBBLESTONE)
                .name(" ${Chat.primaryColor}<bold>Arena Blocks")
            ...
            gui.item(12, tags).onClick runnable@ {
                ...
                for (v in profile.unlockedTags) {
                    ...
                    val display = ItemBuilder(tag.item)
                        .name("<light_purple>${v.lowercase().replaceFirstChar { it.uppercase() }}")
