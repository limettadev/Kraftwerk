...
class BackButton @ConstructorProperties(value = ["back"]) constructor(private val back: Menu?) : Button() {
    ...
    override fun getName(player: Player): String {
        return Chat.colored("&c" + if (back == null) "Close" else "Back")
    }

    override fun getDescription(player: Player): List<String> {
        val lines: MutableList<String> = ArrayList()
        if (back != null) {
            lines.add(Chat.colored("&7Click here to return to"))
            lines.add(Chat.colored("&7the previous menu."))
        } else {
            lines.add(Chat.colored("&7Click here to"))
            lines.add(Chat.colored("&7close this menu."))
        }
        return lines
    }
